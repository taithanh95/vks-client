...
export class OccupationSearchComponent implements OnInit {
  ...
  loadDataFromServer(): void {
    if (this.enabledAutoLoadData) {
      ...
      this.loading = true;
      this.categoriesService.getListOccupation(payload).subscribe(res => {
        if (res) {
  ...
  doDelete() {
    this.categoriesService.deleteOccupation(this.selectedItem?.occuid).subscribe(res => {
      if (res) {
        this.notificationService.showNotification(Constant.ERROR, 'Bạn không thể xóa Nghề nghiệp này do ràng buộc dữ liệu');
      } else {
