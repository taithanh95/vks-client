...
@Component({
  selector: 'app-penalty-search',
  templateUrl: './penalty-search.component.html',
  styleUrls: ['./penalty-search.component.scss']
})
export class PenaltySearchComponent implements OnInit {
  /* Show Dialogs*/
  ...
  loadDataFromServer(): void {
    if (this.enabledAutoLoadData) {
      ...
      this.loading = true;
      this.generalService.searchPenalty(payload).subscribe(res => {
        this.loading = false;
  ...
  doDelete() {
    this.generalService.deletePenalty({
      penaltyid: this.selectedItem.penaltyid
    }).subscribe(res => {
      if (res) {
        const msg = this.generalService.readPropertiesJava(res);
        this.notificationService.showNotification(Constant.ERROR, msg);
      } else {
        this.notificationService.showNotification(Constant.SUCCESS, 'Xóa hình phạt thành công');
        this.loadDataFromServer();
      }
    }, error => {
  ...
  f(value): any {
    return value === 'Chinh' ? 'Chính' : value === 'Bo sung' ?
      'Bổ sung' : value === 'Khac' ? 'Khác' : '';
  }
}
