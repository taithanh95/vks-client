...
export class PartySearchComponent implements OnInit {
  ...
  loadDataFromServer(): void {
    if (this.enabledAutoLoadData) {
      ...
      this.loading = true;
      this.generalService.searchParty(payload).subscribe(res => {
        if(res){
  ...
  doDelete() {
    this.generalService.deleteParty({
      ...
      if (res) {
        ...
        this.notificationService.showNotification(Constant.SUCCESS, 'Xóa chức vụ Đảng thành công');
        this.loadDataFromServer();
      }
    }, error => {
