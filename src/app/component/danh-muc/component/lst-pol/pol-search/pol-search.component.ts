...
@Component({
  selector: 'app-pol-search',
  templateUrl: './pol-search.component.html',
  styleUrls: ['./pol-search.component.scss']
})
export class PolSearchComponent implements OnInit {
  /* Show Dialogs*/
  ...
  loadDataFromServer(): void {
    if (this.enabledAutoLoadData) {
      ...
      this.loading = true;
      this.generalService.searchPol(payload).subscribe(res => {
        this.loading = false;
        if (res) {
  ...
  doDelete() {
    this.generalService.deletePol({
      polid: this.selectedItem.polid
    }).subscribe(res => {
      if (res) {
        const msg = this.generalService.readPropertiesJava(res);
        this.notificationService.showNotification(Constant.ERROR, msg);
      } else {
        this.notificationService.showNotification(Constant.SUCCESS, 'Xóa Cơ quan điều tra thành công');
        this.loadDataFromServer();
      }
    }, error => {
