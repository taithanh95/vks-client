...
@Component({
  selector: 'app-law-group-search',
  templateUrl: './law-group-search.component.html',
  styleUrls: ['./law-group-search.component.scss']
})
export class LawGroupSearchComponent implements OnInit {
  /* Show Dialogs*/
  ...
  loadDataFromServer(): void {
    if (this.enabledAutoLoadData) {
      ...
      this.loading = true;
      this.generalService.searchLawGroup(payload).subscribe(res => {
        if (res) {
  ...
  doDelete() {
    this.generalService.deleteLawGroup({
      ...
      if (res) {
        ...
        this.notificationService.showNotification(Constant.SUCCESS, 'Xóa chương luật thành công');
        this.loadDataFromServer();
      }
    }, error => {
