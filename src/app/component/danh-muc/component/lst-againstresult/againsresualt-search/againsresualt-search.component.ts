...
@Component({
  selector: 'app-againsresualt-search',
  templateUrl: './againsresualt-search.component.html',
  styleUrls: ['./againsresualt-search.component.scss']
})
export class AgainsresualtSearchComponent implements OnInit {
  ...
  loadDataFromServer(): void {
    if (this.enabledAutoLoadData) {
      ...
      this.loading = true;
      this.generalService.getLstAgainstResult(payload).subscribe(res => {
        this.loading = false;
        this.datas = res;
  ...
  doDelete() {
    this.generalService.deleteLstAgainstResult(this.selectedItem?.resultid).subscribe(res => {
      if (res) {
