...
@Component({
  selector: 'app-lawgroupchap-search',
  templateUrl: './lawgroupchap-search.component.html',
  styleUrls: ['./lawgroupchap-search.component.scss']
})
export class LawgroupchapSearchComponent implements OnInit {
  ...
  loadDataFromServer(): void {
    if (this.enabledAutoLoadData) {
      ...
      this.loading = true;
      this.generalService.searchLstLawGroupChap(payload).subscribe(res => {
        if (res) {
  ...
  doDelete() {
    ...
    this.generalService.deleteLstLawGroupChap(payload).subscribe(res => {
      if (res) {
        this.notificationService.showNotification(Constant.ERROR, 'Không thể xóa bản ghi do đã được sử dụng tại chức năng quản lý án hình sự');
      } else {
