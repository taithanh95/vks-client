...
@Component({
  selector: 'app-army-search',
  templateUrl: './army-search.component.html',
  styleUrls: ['./army-search.component.scss']
})
export class ArmySearchComponent implements OnInit {
  /* Show Dialogs*/
  ...
  mapOfExpandedData: { [armyid: string]: TreeNodeInterface[] } = {};
  ...
  loadDataFromServer(): void {
    if (this.enabledAutoLoadData) {
      ...
      this.loading = true;
      this.generalService.searchArmy(payload).subscribe(res => {
        this.loading = false;
        if (res) {
  ...
  toggleButtons() {
    ...
    this.isInsBtn = (this.selectedItem.armylevel === 1 || this.selectedItem.armylevel === 2) ? false : true;
  }

  doDelete() {
    this.generalService.deleteArmy({
      armyid: this.selectedItem.armyid,
      armyparent: this.selectedItem.armyparent
    }).subscribe(res => {
      if (res && res  !== 'success') {
        this.notificationService.showNotification(Constant.ERROR, 'Không thể xóa khi có ràng buộc dữ liệu.');
      } else {
        this.notificationService.showNotification(Constant.SUCCESS, 'Xóa Đơn vị Quân đội thành công');
        if (this.isViewTable)
          this.loadDataFromServer();
        else
          this.getLstTreeData();
   ...
   async getLstTreeData() {
    ...
    this.getTreeData(payload).subscribe(res => {
      if (res) {
        ...
        for (const dt of res) {
          this.mapOfExpandedData[dt.armyid] = this.convertTreeToList(dt);
        }
        this.loadTree = false;
      }
  ...
  getTreeData(payload) {
    return this.categoriesService.getTreeDataArmy({ ...payload, sppid: this.sppId });
  }
  ...
  async collapse(array: TreeNodeInterface[], data: TreeNodeInterface, $event: boolean) {
    if ($event) {
      if (!data.children) {
        this.loadTree = true;
        let level = +data.armylevel;
        const childload = { id: data.armyid, level: ++level };
        this.getTreeData(childload).subscribe(res => {
          if (res) {
            data.children = res;
            const node = this.getParentTreeData(data);
            this.mapOfExpandedData[node.armyid] = this.convertTreeToList(node, true);
            this.loadTree = false;
          }
        }, err => { console.log(err), this.loadTree = false });
      }
      ...
      if (data.pluschildren === 'Y') {
        if (data.children) {
          data.children.forEach(d => {
            const target = array?.find(a => a.armyid === d.armyid);
            target.expand = false;
  ...
  visitNode(node: TreeNodeInterface, hashMap: { [armyid: string]: boolean }, array: TreeNodeInterface[]): void {
    if (!hashMap[node.armyid]) {
      hashMap[node.armyid] = true;
      array.push(node);
    }
  }
