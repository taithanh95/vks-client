...
@Component({
  selector: 'app-nation-search',
  templateUrl: './nation-search.component.html',
  styleUrls: ['./nation-search.component.scss']
})
export class NationSearchComponent implements OnInit {
  ...
  loadDataFromServer(): void {
    if (this.enabledAutoLoadData) {
      ...
      this.loading = true;
      this.categoriesService.getListNation(payload).subscribe(res => {
        if (res) {
  ...
  doDelete() {
    this.generalService.deleteLstNation(this.selectedItem?.natiid).subscribe(res => {
      if (res) {
        this.handleErr(res);
      } else {
