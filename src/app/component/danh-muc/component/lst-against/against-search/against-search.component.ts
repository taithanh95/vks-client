...
@Component({
  selector: 'app-against-search',
  templateUrl: './against-search.component.html',
  styleUrls: ['./against-search.component.scss']
})
export class AgainstSearchComponent implements OnInit {
  ...
  loadDataFromServer(): void {
    if (this.enabledAutoLoadData) {
      ...
      this.loading = true;
      this.categoriesService.getLstAgainstName(payload).subscribe(res => {
        if (res) {
  ...
  doDelete() {
    this.generalService.deleteLstAgainst(this.selectedItem?.againstid).subscribe(res => {
      if (res) {
