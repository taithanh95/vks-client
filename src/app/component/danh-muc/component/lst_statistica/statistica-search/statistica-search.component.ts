...
@Component({
  selector: 'app-statistica-search',
  templateUrl: './statistica-search.component.html',
  styleUrls: ['./statistica-search.component.scss']
})
export class StatisticaSearchComponent implements OnInit {
  ...
  loadDataFromServer(): void {
    if (this.enabledAutoLoadData) {
      ...
      this.loading = true;
      this.generalService.getLstStatistica(payload).subscribe(res => {
        this.loading = false;
        this.datas = res;
  ...
  doDelete() {
    this.generalService.deleteLstStatistica(this.selectedItem?.STATID).subscribe(res => {
      if (res) {
