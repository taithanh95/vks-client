...
@Component({
  selector: 'app-customs-edit',
  templateUrl: './customs-edit.component.html',
  styleUrls: ['./customs-edit.component.scss']
})
export class CustomsEditComponent implements OnInit {
  ...
  handleSubmit(data) {
    ...
    this.generalService.updateCustoms(payload).subscribe(res => {
      if (res) {
        this.notificationService.showNotification(Constant.SUCCESS, 'Cập nhật Đơn vị hải quan thành công');
        this.handleCancel();
        this.reload.emit();
        this.resetData();
      } else {
