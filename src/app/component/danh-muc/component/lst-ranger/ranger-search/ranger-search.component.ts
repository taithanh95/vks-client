...
@Component({
  selector: 'app-ranger-search',
  templateUrl: './ranger-search.component.html',
  styleUrls: ['./ranger-search.component.scss']
})
export class RangerSearchComponent implements OnInit {
  /* Show Dialogs*/
  ...
  mapOfExpandedData: { [rangid: string]: TreeNodeInterface[] } = {};
  ...
  loadDataFromServer(): void {
    if (this.enabledAutoLoadData) {
      ...
      this.loading = true;
      this.generalService.searchRanger(payload).subscribe(res => {
        this.loading = false;
        if (res) {
  ...
  doDelete() {
    this.generalService.deleteRanger({
      rangid: this.selectedItem.rangid,
      rangparent: this.selectedItem.rangparent
    }).subscribe(res => {
      if (res && res  !== 'success') {
        this.notificationService.showNotification(Constant.ERROR, 'Không thể xóa khi có ràng buộc dữ liệu.');
      } else {
        this.notificationService.showNotification(Constant.SUCCESS, 'Xóa Đơn vị Kiểm lâm thành công');
        if (this.isViewTable)
          this.loadDataFromServer();
        else
          this.getLstTreeData();
  ...
  async getLstTreeData() {
    ...
    this.getTreeData(payload).subscribe(res => {
      if (res) {
        ...
        for (const dt of res) {
          this.mapOfExpandedData[dt.rangid] = this.convertTreeToList(dt);
        }
        this.loadTree = false;
      }
  ...
  getTreeData(payload) {
    return this.categoriesService.getTreeDataRanger({ ...payload, sppid: this.sppId });
  }
  ...
  async collapse(array: TreeNodeInterface[], data: TreeNodeInterface, $event: boolean) {
    if ($event) {
      if (!data.children) {
        this.loadTree = true;
        let level = +data.ranglevel;
        const childload = { id: data.rangid, level: ++level };
        this.getTreeData(childload).subscribe(res => {
          if (res) {
            data.children = res;
            const node = this.getParentTreeData(data);
            this.mapOfExpandedData[node.rangid] = this.convertTreeToList(node, true);
            this.loadTree = false;
          }
        }, err => { console.log(err), this.loadTree = false });
      }
      ...
      if (data.pluschildren === 'Y') {
        if (data.children) {
          data.children.forEach(d => {
            const target = array?.find(a => a.rangid === d.rangid);
            target.expand = false;
  ...
  visitNode(node: TreeNodeInterface, hashMap: { [rangid: string]: boolean }, array: TreeNodeInterface[]): void {
    if (!hashMap[node.rangid]) {
      hashMap[node.rangid] = true;
      array.push(node);
    }
  }
