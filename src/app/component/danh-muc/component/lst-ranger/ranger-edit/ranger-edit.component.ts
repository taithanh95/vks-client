...
@Component({
  selector: 'app-ranger-edit',
  templateUrl: './ranger-edit.component.html',
  styleUrls: ['./ranger-edit.component.scss']
})
export class RangerEditComponent implements OnInit {
  ...
  handleSubmit(data) {
    ...
    this.generalService.updateRanger(payload).subscribe(res => {
      if (res) {
        this.notificationService.showNotification(Constant.SUCCESS, 'Cập nhật Đơn vị kiểm lâm thành công');
        this.handleCancel();
        this.reload.emit();
        this.resetData();
      } else {
