...
@Component({
  selector: 'app-spc-search',
  templateUrl: './spc-search.component.html',
  styleUrls: ['./spc-search.component.scss']
})
export class SpcSearchComponent implements OnInit {
  /* Show Dialogs*/
  ...
  mapOfExpandedData: { [spcid: string]: TreeNodeInterface[] } = {};
  ...
  loadDataFromServer(): void {
    if (this.enabledAutoLoadData) {
      ...
      this.loading = true;
      this.generalService.searchSPC(payload).subscribe(res => {
        this.loading = false;
        if (res) {
  ...
  toggleButtons() {
    ...
    this.isInsBtn = (this.selectedItem.spclevel === '1' || this.selectedItem.spclevel === '2') ? false : true;
  }

  doDelete() {
    this.generalService.deleteSPC({
      spcid: this.selectedItem.spcid,
      spcparent: this.selectedItem.spcparent
    }).subscribe(res => {
      if (res && res  !== 'success') {
        this.notificationService.showNotification(Constant.ERROR, 'Không thể xóa khi có ràng buộc dữ liệu.');
      } else {
        this.notificationService.showNotification(Constant.SUCCESS, 'Xóa Tòa án thành công');
        if (this.isViewTable)
          this.loadDataFromServer();
        else
          this.getLstTreeData();
   ...
   */
   ...
   async getLstTreeData() {
    this.loadTree = true;
    const payload = { spcid: '', level: '0' }
    this.getTreeData(payload).subscribe(res => {
      if (res) {
        this.listOfMapData = res;
        for (const dt of res) {
          this.mapOfExpandedData[dt.spcid] = this.convertTreeToList(dt);
        }
        this.loadTree = false;
      }
  ...
  getTreeData(payload) {
    return this.categoriesService.getTreeDataSPC({ ...payload, sppid: this.sppId });
  }
  ...
  async collapse(array: TreeNodeInterface[], data: TreeNodeInterface, $event: boolean) {
    if ($event) {
      if (!data.children) {
        this.loadTree = true;
        let level = +data.spclevel;
        const childload = { spcid: data.spcid, level: ++level };
        this.getTreeData(childload).subscribe(res => {
          if (res) {
            data.children = res;
            const node = this.getParentTreeData(data);
            this.mapOfExpandedData[node.spcid] = this.convertTreeToList(node, true);
            this.loadTree = false;
          }
        }, err => { console.log(err), this.loadTree = false });
      }
      ...
      if (data.pluschildren === 'Y') {
        if (data.children) {
          data.children.forEach(d => {
            const target = array?.find(a => a.spcid === d.spcid);
            target.expand = false;
  ...
  visitNode(node: TreeNodeInterface, hashMap: { [locaid: string]: boolean }, array: TreeNodeInterface[]): void {
    if (!hashMap[node.spcid]) {
      hashMap[node.spcid] = true;
      array.push(node);
    }
  }
