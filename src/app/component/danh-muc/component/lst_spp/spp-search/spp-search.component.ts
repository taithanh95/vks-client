...
@Component({
  selector: 'app-spp-search',
  templateUrl: './spp-search.component.html',
  styleUrls: ['./spp-search.component.scss']
})
export class SppSearchComponent implements OnInit {
  /* Show Dialogs*/
  ...
  mapOfExpandedData: { [sppid: string]: TreeNodeInterface[] } = {};
  ...
  loadDataFromServer(): void {
    if (this.enabledAutoLoadData) {
      ...
      this.loading = true;
      this.generalService.searchSPP(payload).subscribe(res => {
        this.loading = false;
        if (res) {
  ...
  toggleButtons() {
    ...
    this.isInsBtn = (this.selectedItem.sppLevel === '1' || this.selectedItem.sppLevel === '2') ? false : true;
  }

  doDelete() {
    this.generalService.deleteSPP({
      sppid: this.selectedItem.sppId,
      sppparent: this.selectedItem.sppParent
    }).subscribe(res => {
      if (res && res  !== 'success') {
        this.notificationService.showNotification(Constant.ERROR, 'Không thể xóa khi có ràng buộc dữ liệu.');
      } else {
        this.notificationService.showNotification(Constant.SUCCESS, 'Xóa Viện kiểm sát thành công');
        if (this.isViewTable)
          this.loadDataFromServer();
        else
          this.getLstTreeData();
   ...
   async getLstTreeData() {
    ...
    this.getTreeData(payload).subscribe(res => {
      if (res) {
        ...
        for (const dt of res) {
          this.mapOfExpandedData[dt.sppId] = this.convertTreeToList(dt);
        }
        this.loadTree = false;
      }
  ...
  getTreeData(payload) {
    return this.categoriesService.getTreeDataSPP({ ...payload, sppid: this.sppId });
  }
  ...
  async collapse(array: TreeNodeInterface[], data: TreeNodeInterface, $event: boolean) {
    if ($event) {
      if (!data.children) {
        this.loadTree = true;
        let level = +data.sppLevel;
        const childload = { id: data.sppId, level: ++level };
        this.getTreeData(childload).subscribe(res => {
          if (res) {
            data.children = res;
            const node = this.getParentTreeData(data);
            this.mapOfExpandedData[node.sppId] = this.convertTreeToList(node, true);
            this.loadTree = false;
          }
        }, err => { console.log(err), this.loadTree = false });
      }
      ...
      if (data.pluschildren === 'Y') {
        if (data.children) {
          data.children.forEach(d => {
            const target = array?.find(a => a.sppId === d.sppId);
            target.expand = false;
  ...
  visitNode(node: TreeNodeInterface, hashMap: { [sppid: string]: boolean }, array: TreeNodeInterface[]): void {
    if (!hashMap[node.sppId]) {
      hashMap[node.sppId] = true;
      array.push(node);
    }
  }
