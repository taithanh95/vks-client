...
@Component({
  selector: 'app-resolve-search',
  templateUrl: './resolve-search.component.html',
  styleUrls: ['./resolve-search.component.scss']
})
export class ResolveSearchComponent implements OnInit {
  ...
  loadDataFromServer(): void {
    if (this.enabledAutoLoadData) {
      ...
      this.loading = true;
      this.generalService.getLstResolve(payload).subscribe(res => {
        if (res) {
  ...
  doDelete() {
    this.generalService.deleteLstResolve(this.selectedItem?.resolid).subscribe(res => {
      if (res) {
