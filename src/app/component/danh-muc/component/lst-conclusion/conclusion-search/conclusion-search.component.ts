...
@Component({
  selector: 'app-conclusion-search',
  templateUrl: './conclusion-search.component.html',
  styleUrls: ['./conclusion-search.component.scss']
})
export class ConclusionSearchComponent implements OnInit {
  ...
  loadDataFromServer(): void {
    if (this.enabledAutoLoadData) {
      ...
      this.loading = true;
      this.generalService.getLstConclusion(payload).subscribe(res => {
        if (res) {
  ...
  doDelete() {
    this.generalService.deleteLstConclusion(this.selectedItem?.concid).subscribe(res => {
      if (res) {
