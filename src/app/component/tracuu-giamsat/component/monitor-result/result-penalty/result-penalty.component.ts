...
@Component({
  selector: 'app-result-penalty',
  templateUrl: './result-penalty.component.html',
  styleUrls: ['./result-penalty.component.scss']
})
export class ResultPenaltyComponent implements OnInit {
  ...
  doSearch(): void {
    ...
    const payload = {
      ...this.filterItem,
      sppidList: listSppid,
      usefor: this.filterItem?.userforPenalty,
      penaltyid: this.filterItem?.penaltyid,
      pageSize: this.defaultPage,
      pageIndex: this.defaultPage * (this.pageIndex - 1)
    }
    this.loading = true;
    this.generalService.searchPenaltys(payload).subscribe(res => {
      this.loading = false;
      this.data = res;
