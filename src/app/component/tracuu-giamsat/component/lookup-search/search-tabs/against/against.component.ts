...
export class AgainstComponent implements OnInit {
  ...
  loadDataFromServer(): void {
    if (this.enabledAutoLoadData) {
      ...
      this.loading = true;
      this.generalService.searchLookupAgainst(payload).subscribe(res => {
        this.loading = false;
        this.datas = res;
  ...
  onValueFDate(event: any) {
    this.filterItem.fromdateagainst = this.datechangeService.onDateValueChange(event);
  }

  onValueTDate(event: any) {
    this.filterItem.todateagainst = this.datechangeService.onDateValueChange(event);
  }

  checkDate(): void {
    if (this.filterItem.fromdateagainst && this.filterItem.todateagainst) {
      const fromdate = new Date(this.filterItem.fromdateagainst);
      const todate = new Date(this.filterItem.todateagainst);
      const count = WebUtilities.calculateDiff(fromdate, todate);
      if (count > 0) {
        this.notificationService.showNotification(Constant.ERROR, 'Ngày kháng nghị đến ngày phải lớn hơn hoặc bằng Ngày kháng nghị từ ngày');
        this.filterItem.todateagainst = null;
      }
    }
  }
}
