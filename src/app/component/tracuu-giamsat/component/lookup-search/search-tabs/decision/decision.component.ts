...
export class DecisionComponent implements OnInit {
  ...
  loadDataFromServer(): void {
    if (this.enabledAutoLoadData) {
      ...
      this.loading = true;
      this.generalService.searchLookupDecision(payload).subscribe(res => {
        this.loading = false;
        this.datas = res;
  ...
  onValueFDate(event: any) {
    this.filterItem.fromdatedeci = this.datechangeService.onDateValueChange(event);
  }

  onValueTDate(event: any) {
    this.filterItem.todatedeci = this.datechangeService.onDateValueChange(event);
  }

  checkDate(): void {
    if (this.filterItem.fromdatedeci && this.filterItem.todatedeci) {
      const fromdate = new Date(this.filterItem.fromdatedeci);
      const todate = new Date(this.filterItem.todatedeci);
      const count = WebUtilities.calculateDiff(fromdate, todate);
      if (count > 0) {
        this.notificationService.showNotification(Constant.ERROR, 'Ngày quyết định đến ngày phải lớn hơn hoặc bằng Ngày quyết định từ ngày');
