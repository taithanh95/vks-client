...
export class PenaltyComponent implements OnInit {
  ...
  loadDataFromServer(): void {
    if (this.enabledAutoLoadData) {
      ...
      const payload = {
        ...
        sppid: '',
        penaltyid: this.filterItem?.penaltyid?.PENALTYID
      }
      this.loading = true;
      this.generalService.searchLookupPenalty(payload).subscribe(res => {
        this.loading = false;
        this.datas = res;
  ...
  checkDate(): void {
    if (this.filterItem.fromdatecen && this.filterItem.todatecen) {
      const fromdate = new Date(this.filterItem.fromdatecen);
      const todate = new Date(this.filterItem.todatecen);
      const count = WebUtilities.calculateDiff(fromdate, todate);
      if (count > 0) {
        this.notificationService.showNotification(Constant.ERROR, 'Bản án đến ngày phải lớn hơn hoặc bằng Bản án từ ngày');
        this.filterItem.todatecen = null;
      }
    }
  }
