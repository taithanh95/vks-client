...
@Component({
  selector: 'app-detail-inspector',
  templateUrl: './detail-inspector.component.html',
  styleUrls: ['./detail-inspector.component.scss']
})
export class DetailInspectorComponent implements OnInit {
  @Input() tabs: any;
  ...
  getListAccused(): void {
    ...
    this.generalService.getListLookupInspector(payload).subscribe(res => {
      this.loading = false;
      this.lstInspector = res;
      if (this.lstInspector.length != 0)
        this.total = this.lstInspector[0].ROWCOUNT;
      else
        this.total = 0;
    }, error => {
