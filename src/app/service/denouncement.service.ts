...
export class DenouncementService extends BaseService {
  ...
  search(Obj: any,page,size,sort): Observable<any> {
    return this.post(`/denouncement/search?page=${page}&size=${size}&sort=${sort}`,Obj);
  }
  ...
  updateDenouncement(data: DenouncementModel) {
    ...
    return this.post(`/denouncement/updateDenouncement?sppId=${sppId}`, data);
  }
